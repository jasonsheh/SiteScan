...
class GitScan:
    def __init__(self, keyword):
        ...
        self.hash_list = []
        self.useless_ext = ['css', 'htm', 'html', 'pac', 'csv']
        self.keywords = ['root password', 'smtp password', 'admin password']
        self.fuzz_list = ['fuzz', 'dict', 'hack']
        self.email = ['@163.com']
        ...
        for kw in self.keywords:
            ...
            for index, content in enumerate(resource.get_page(1)):
                if content.sha not in self.hash_list:
                    ...
                    if self.remove_dict_from_repo(full_name):
                        continue
                    if self.filter(url):
                        continue
                    try:
